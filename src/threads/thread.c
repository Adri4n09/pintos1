...
/*
 * Testeaza daca threadul curent are cea mai mare prioritate,
 * daca nu atunci schimba threadul.
 *
 */
void refresh_priority() {

	struct thread *t_c = thread_current();
	t_c->priority = t_c->init_priority;
	if (list_empty(&t_c->donations)) {
		return;
	}
	struct thread *d = list_entry(list_front(&t_c->donations), struct thread,
			donation_elem);
	if (d->priority > t_c->priority) {
		t_c->priority = d->priority;
	}
}
